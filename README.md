...
| 业务身份 | 默认账号 | 默认密码 | 指标范围 | 机构范围 |
|---|---|---|---|---|
| 总行管理层 | `analyst` | `analyst123` | ZB001～ZB021（全部21项） | ORG001～ORG013（A～M市全部机构） |
| 分支行管理层 | `analyst2` | `analyst2123` | ZB001～ZB021（全部21项） | 仅 ORG001（江苏省A市农商行） |
| 业务条线人员 | `analyst3` | `analyst3123` | ZB003～ZB006、ZB018～ZB021 | 仅 ORG001（江苏省A市农商行） |
| 风险合规人员 | `risk` | `risk123` | ZB013～ZB017 | ORG001～ORG013（A～M市全部机构） |
| 财务人员 | `finance` | `finance123` | ZB001～ZB002、ZB007～ZB012 | ORG001～ORG013（A～M市全部机构） |
| 系统管理员 | `admin` | `admin123` | 无业务指标查询权限 | 无业务机构范围；可管理用户、告警和审计，并查看查询问题，但不展示答案和查询结果 |

指标编号的具体含义如下：

- ZB001～ZB006：各项存款余额、各项贷款余额、对公存款余额、个人存款余额、对公贷款余额、个人贷款余额；
- ZB007～ZB012：中间业务收入、净利息收入、营业收入、营业支出、净利润、成本收入比；
- ZB013～ZB017：不良贷款率、不良贷款余额、拨备覆盖率、资本充足率、逾期贷款率；
- ZB018～ZB021：员工人数、网点数量、个人客户数、对公客户数。

机构编号按字母顺序对应：ORG001 为江苏省A市农商行，ORG002 为B市，依次类推，
ORG013 为M市。`analyst2` 和 `analyst3` 当前默认都只允许访问A市农商行；如需演示
其他机构，可分别修改 `TEXT2SQL_ANALYST_2_ORGS` 和 `TEXT2SQL_ANALYST_3_ORGS`，
多个机构编号使用英文逗号分隔。
